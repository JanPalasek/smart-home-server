...
{
    public class UpdateSensorService : IUpdateSensorService
    {
        private readonly ISensorRepository repository;

        public UpdateSensorService(ISensorRepository repository)
        {
            this.repository = repository;
        }

        public async Task<ServiceResult<long>> UpdateSensorAsync(SensorModel model)
        {
            if (model.Id <= 0)
            {
